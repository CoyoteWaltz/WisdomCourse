...
- - 解答：点操作符: 静态的。右侧必须是一个以属性名称命名的简单**标识符(identifier)**。属性名用一个标识符来表示。标识符必须直接出现在 js 程序中，它们不是数据类型，因此程序无法修改它们。所以 `obj.i` 的 `i` 只是一个 identifier 并不是变量，所以 eslint 会提示没有用到 i
